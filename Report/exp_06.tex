% Experiment 6: Signal-Based Timeout Supervisor

\section{Experiment 6: Signal-Based Timeout Supervisor}

\subsection{Problem Statement}
The objective is to implement a CLI tool named \texttt{timeoutwrap} that enforces a timeout on a child process.

The tool must:
\begin{itemize}
    \item Accept inputs \verb|--seconds <t>| and \verb|--cmd <program>|.
    \item Run the command in a child process.
    \item If the child exits before timeout, print \verb|OK: EXIT <code>|.
    \item If timeout occurs, kill the child and print \verb|OK: TIMEOUT KILLED|.
    \item Use \verb|sigaction| and \verb|alarm|.
\end{itemize}

\subsection{Source Code}
\begin{lstlisting}[language=C, caption={timeoutwrap.c}]
// ##########  Timeout Supervisor ########
