% Experiment 3: Directory Listing and Metadata Report

\section{Experiment 3: Directory Listing and Metadata Report}

\subsection{Problem Statement}
The objective is to implement a CLI tool named \texttt{dirreport} that lists directory contents and reports metadata.

The tool must:
\begin{itemize}
    \item Accept input \verb|--path <dir>| (required) and optional \verb|--sort name| or \verb|--sort size|.
    \item Output one line per entry: \verb|ENTRY <type> <size> <name>|.
    \item Classify types as 'F' (file), 'D' (directory), 'L' (link), or 'O' (other).
    \item Print a summary line: \verb|OK: TOTAL <n> FILES <f> DIRS <d> LINKS <l> OTHER <o>|.
    \item Handle errors with codes like \verb|E_NOTDIR|, \verb|E_OPEN_DIR|.
\end{itemize}

\subsection{Source Code}
\begin{lstlisting}[language=C, caption={dirreport.c}]
...
\textbf{Case 1:}
\begin{verbatim}
./exp_03 --path testdir --sort                          
ENTRY F 0 test1
ENTRY D 4096 test2
OK: TOTAL 2 FILES 1 DIRS 1 LINKS 0 OTHER 0
\end{verbatim}

\textbf{Case 2:}
\begin{verbatim}
$ ./exp_03 --path fixtures/q03/file.txt
